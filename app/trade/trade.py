...
class TradeManager:
    ...
    def __init__(self, trade_json):
        ...
        self.itens_right = Inventory(trade_right_json['itens'])
    ...
    def equalPoints(self):
        if self.calcPoints(self.items_left) == self.calcPoints(self.itens_right):
            return True
        return False
    ...
    def trade(self):
        if self.traderLeft is not None and  self.traderRight is not None:
            ...
            if self.equalPoints() and self.traderLeft.canTrade() and self.traderRight.canTrade():
                ...
                if self.traderLeft.canGiveItens(self.items_left) and self.traderRight.canGiveItens(self.itens_right):

                    new_inventory_left = self.traderLeft.performTrade(self.items_left,self.itens_right)
                    new_inventory_right = self.traderRight.performTrade(self.items_left,self.items_left)
                ...
                json = {'error':'trade is only possible with same quantity of points'}
                return jsonify(json), 409
        else:
